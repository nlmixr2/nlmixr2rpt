...
When you create the `report_fit.yaml` file you have defined figures and tables for reporting. However you may want to reuse these report objects outside of Word and PowerPoint. For example you may want to use them inside of Shiny Apps or in RMarkdown document. This vignette outlines how you can build the objects outside of the `rptnlmixr` workflow. 
...
You should have an `nlmixr` fit object from your own analysis. We will load an example from this package.
...
rptdetails = yaml_read_fit(
  obnd    = obnd,
  rptyaml = system.file(package="rptnlmixr", "examples", "report_fit_test.yaml"),
  fit     = fit)$rptdetails
```
...
tab_ft = flextable::set_caption(tab_ft, caption = tab_title)
